  ...
  const showrooms=()=>{
    if(room!==""&&sday!==""&&eday!==""){
      ...
      if(room==='Single Bed Room'){
       setCount(Array(parseInt(data.signlebed)).fill(1).map((x, y) => x + y))
      }
      else if(room==='Double Bed Room'){
        // Array(parseInt(30)).fill(1).map((x, y) => x + y)
       setCount(Array(parseInt(data.doublebed)).fill(1).map((x, y) => x + y))
    }
      else{
        setCount(Array(parseInt(data.kingsizebed)).fill(1).map((x, y) => x + y));
      }
    }
  else{
...
const handleRoomno=(e)=>{
  ...
  <Grid xs={2} md={2} sm={2}></Grid>
        <Grid xs={8} md={8} sm={8} item style={{ boxShadow: '0 0 15px  #545001' }}>
          <Card>
            ...
            <Grid xs={12} md={12} sm={12} item style={{ textAlign: 'left', paddingLeft: '40px',marginRight:'20px' }}>
              <p>
                <span>
                  <b>Address: </b>
                </span>
              ...
              {user===true?<h2 style={{textAlign:'center', textDecoration:'underline', padding:"20px"}}>Book Your Room</h2>:null}
              <Grid container>
              {user===false?<p>
                <span>
              ...
              <Grid xs={12} md={12} sm={12} style={{marginTop:'10px'}}>
                ...
                <Select
                  ...
                  onChange={handleroom}
                >
                          <MenuItem value={'Single Bed Room'}>Single Bed Room</MenuItem>
                          <MenuItem value={'Double Bed Room'}>Double Bed Room</MenuItem>
                          <MenuItem value={'King Size Bed Room'}>King Size Bed Room</MenuItem>                        
                </Select>
              </Grid>
              }
              {user===false?<p>
            ...
            :null}
              ...
              <p style={{textAlign:"center"}}>
                {user===false?
              <Button style={{backgroundColor:'#545001' ,color:'#fff',marginBottom:'20px'}} variant="contained" onClick={()=>history.push('/login')}>
              Book Your Room
           </Button>:slots===false?
             <Button style={{backgroundColor:'#545001' ,color:'#fff',marginBottom:'20px'}} variant="contained" onClick={()=>showrooms()}>
              Show rooms
           </Button>:
                <Grid xs={12} md={12} sm={12} style={{marginTop:'10px'}}>
                ...
                >
                  {count.map((val,ind)=>{
                    return(
                  <MenuItem value={val} key={ind}>Room No {val}</MenuItem>                        
                    )
                  })}
                </Select>
               {card===true?
